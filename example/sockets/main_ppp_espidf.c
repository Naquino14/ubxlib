...
static const uNetworkCfgCell_t gNetworkCfg = {
    ...
    //   authentication mode that should be used, see see
    //   #uCellNetAuthenticationMode_t in u_cell_net.h;, noting
                ...
                if (sock >= 0) {
                    if (connect(sock, (struct sockaddr *) &destinationAddress, sizeof(destinationAddress)) == 0) {
                        if (send(sock, message, txSize, 0) == txSize) {
                            ...
                            uPortLog("Unable to send to server!\n");
                        }
                    } else {
                        uPortLog("Unable to connect to server!\n");
                    }
                } else {
                    uPortLog("Unable to create socket!\n");
                }
