...
static esp_err_t postAttachStart(esp_netif_t *pEspNetif, void *pArgs)
{
    ...
    if (espError == ESP_OK) {
        ...
        esp_netif_auth_type_t authenticationType = NETIF_PPP_AUTHTYPE_PAP;
        if ((pDriver->pUsername != NULL) && (pDriver->pPassword != NULL)) {
            // The enumeration used by ESP-IDF matches uPortPppAuthenticationMode_t
            authenticationType = (esp_netif_auth_type_t) pDriver->authenticationMode;
        }
        // Set the username/password fields to an empty string otherwise the
        // authentication mode will not be accepted
        ...
        if (authenticationMode < U_PORT_PPP_AUTHENTICATION_MODE_MAX_NUM) {
            ...
            if (pPppInterface != NULL) {
                ...
                if (pEspNetif != NULL) {
                    ...
                    pPppInterface->netifDriver.pDnsIpAddressSecondary = pDnsIpAddressSecondary;
...
int32_t uPortPppDisconnect(void *pDevHandle)
{
    ...
    if (gMutex != NULL) {
        ...
        pPppInterface = pFindPppInterface(pDevHandle);
        if (pPppInterface != NULL) {
            // No different from detach, it's going doooooown...
            pppDetach(pPppInterface);
            errorCode = (int32_t) U_ERROR_COMMON_SUCCESS;
        }
