...
/** @file
 * @brief Tests that use ESP-IDF's native sockets API to test the
 * PPP-level integration of ubxlib as a transport of LWIP.  These tests
 * should pass on ESP32 when there is a cellular module connected.
 ...
 *
 * The tests are only compiled if CONFIG_LWIP_PPP_SUPPORT and
 * U_CFG_PPP_ENABLE are defined.
 *
 ...
 */
#define U_TEST_PREFIX "U_ESPIDF_SOCK_TEST: "

/** Print a whole line, with terminator, prefixed for this test file.
 */
#define U_TEST_PRINT_LINE(format, ...) uPortLog(U_TEST_PREFIX format "\n", ##__VA_ARGS__)

#ifndef U_ESPIDF_PPP_TEST_RECEIVE_TASK_STACK_SIZE_BYTES
/** The stack size to use for the asynchronous receive task.
 */
# define U_ESPIDF_PPP_TEST_RECEIVE_TASK_STACK_SIZE_BYTES 2560
#endif

#ifndef U_ESPIDF_PPP_TEST_RECEIVE_TASK_PRIORITY
/** The priority to use for the asynchronous receive task.
 */
# define U_ESPIDF_PPP_TEST_RECEIVE_TASK_PRIORITY (U_CFG_OS_PRIORITY_MIN + 5)
#endif

#ifndef U_ESPIDF_PPP_TEST_RECEIVE_TASK_RELAX_MS
/** How long the receive task should relax for between receive
 * attempts.
 */
# define U_ESPIDF_PPP_TEST_RECEIVE_TASK_RELAX_MS 10
#endif

#ifndef U_ESPIDF_PPP_TEST_RECEIVE_TASK_EXIT_MS
/** How long to allow for the the receive task to exit;
 * should be quite a lot longer than
 * #U_ESPIDF_PPP_TEST_RECEIVE_TASK_EXIT_MS.
 */
# define U_ESPIDF_PPP_TEST_RECEIVE_TASK_EXIT_MS 100
#endif
...
static uEspidfPppSockTestConfig_t gTestConfig = {.taskHandle = NULL};
...
{
    int32_t sizeBytes;
    uEspidfPppSockTestConfig_t *pTestConfig = (uEspidfPppSockTestConfig_t *) pParameter;
    ...
    do {
        ...
        if (sizeBytes > 0) {
            ...
            uPortTaskBlock(U_ESPIDF_PPP_TEST_RECEIVE_TASK_RELAX_MS);
        }
...
static void osCleanup()
{
    if (gTestConfig.taskHandle != NULL) {
        ...
        uPortTaskBlock(U_ESPIDF_PPP_TEST_RECEIVE_TASK_EXIT_MS);
...
/** Basic TCP test.
 */
U_PORT_TEST_FUNCTION("[espidfSock]", "espidfSockTcp")
{
    ...
    for (uNetworkTestList_t *pTmp = pList; pTmp != NULL; pTmp = pTmp->pNext) {
        ...
        if ((pModule == NULL) ||
            ...
            U_TEST_PRINT_LINE("opening socket() to %s:%d returned %d.",
                              hostIp, U_SOCK_TEST_ECHO_TCP_SERVER_PORT, sock);
            ...
            U_TEST_PRINT_LINE("connect() returned %d.", errorCode);
            ...
            U_PORT_TEST_ASSERT(uPortTaskCreate(rxTask, "rxTask",
                                               U_ESPIDF_PPP_TEST_RECEIVE_TASK_STACK_SIZE_BYTES,
                                               (void *) &gTestConfig,
                                               U_ESPIDF_PPP_TEST_RECEIVE_TASK_PRIORITY,
 ...
 */
U_PORT_TEST_FUNCTION("[espidfSock]", "espidfSockCleanUp")
{
    osCleanup();
