...
static void connectPpp(uDeviceHandle_t cellHandle,
                       const char *pUsername,
                       const char *pPassword,
                       uCellNetAuthenticationMode_t authenticationMode)
{
    ...
    // so this is fine
    uPortPppConnect(cellHandle, pIpAddress, pDnsIpAddressPrimary,
                        ...
                        bool (*pKeepGoingCallback) (uDeviceHandle_t cellHandle))
{
    ...
    if (gUCellPrivateMutex != NULL) {
        ...
        // PPP connection _must_ be taken down before this happens
        // and that won't be able to act while the cellular API
        // mutex is locked, so we alays take the PPP connection
        // down first
        ...
        if ((errorCode == 0) && hasPpp) {
            ...
            connectPpp(cellHandle, pUsername, pPassword, authenticationModeUsed);
        }
    }
